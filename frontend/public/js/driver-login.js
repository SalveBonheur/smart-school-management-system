        ...
        form.addEventListener('submit', async function(e) {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    if (rememberMe) {
                        localStorage.setItem('rememberedDriver', JSON.stringify({ email, password }));
                    } else {
                    ...
                    setTimeout(() => {
                        if (response.driver.status === 'pending') {
                            window.location.href = 'driver-pending.html';
                        } else {
...
// Login driver API call
async function loginDriver(email, password) {
    try {
        const response = await fetch('http://localhost:3006/api/auth-new/drivers/login', {
            method: 'POST',
            headers: {
...
async function sendPasswordReset() {
    ...
    try {
        const response = await fetch('/api/drivers/forgot-password', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
